...
DecoderConfig = Union['RecurrentDecoderConfig', transformer.TransformerConfig, 'ConvolutionalDecoderConfig',
                      'RecurrentLanguageModelConfig', transformer.TransformerLanguageModelConfig]
...
@Decoder.register(transformer.TransformerLanguageModelConfig, C.TRANSFORMER_DECODER_PREFIX)
class TransformerLanguageModel(Decoder):
    """
    Transformer Language Model

    :param config: Configuration for Transformer language model.
    :param prefix: Name prefix for symbols of this decoder.
    """

    def __init__(self,
                 config: transformer.TransformerLanguageModelConfig,
                 prefix: str = C.TRANSFORMER_DECODER_PREFIX) -> None:
        super().__init__(config.dtype)
        self.config = config
        self.prefix = prefix
        self.layers = [transformer.TransformerLanguageModelBlock(
            config, prefix="%s%d_" % (prefix, i)) for i in range(config.num_layers)]
        self.final_process = transformer.TransformerProcessBlock(sequence=config.preprocess_sequence,
                                                                 num_hidden=config.model_size,
                                                                 dropout=config.dropout_prepost,
    ...
    def decode_sequence(self,
        ...
        if mx.sym.sum(source_encoded_lengths) < 0:
            # Never happens, a fake use of source_encoded.
            return mx.sym.broadcast_add(target, mx.sym.sum(source_encoded_lengths) * 0)
        else:
            return target
