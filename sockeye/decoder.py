...
DecoderConfig = Union['RecurrentDecoderConfig', transformer.TransformerConfig, 'ConvolutionalDecoderConfig',
                      'RecurrentLanguageModelConfig', 'RecurrentSelfDecoderConfig']
...
class RecurrentSelfDecoderConfig(Config):
    """
    Recurrent self decoder configuration.
...
@Decoder.register(RecurrentSelfDecoderConfig, C.RNN_DECODER_PREFIX)
class RecurrentSelfDecoder(Decoder):
    """
    RNN Decoder with attention.
    The architecture is based on Luong et al, 2015: Effective Approaches to Attention-based Neural Machine Translation.
    ...
    def __init__(self,
                 config: RecurrentSelfDecoderConfig,
                 prefix: str = C.RNN_DECODER_PREFIX) -> None:
        super().__init__(config.dtype)
        # TODO: implement variant without input feeding
        self.config = config
    ...
    def decode_sequence(self,
        ...
        for seq_idx in range(target_embed_max_length):
            # hidden: (batch_size, rnn_num_hidden)
            state, attention_state = self._step(state.hidden,
                                                state,
                                                attention_func,
                                                attention_state,
                                                seq_idx)
...
class ConvolutionalDecoder(Decoder):
    ...
    def decode_sequence(self,
        ...
        target_decoded = self._decode(source_encoded=source_encoded,
